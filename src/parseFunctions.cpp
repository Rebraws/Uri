  ...
  -> std::optional<std::string>
{
  ...
  if (std::next(scheme_end) != uri_str.end() && *std::next(scheme_end) == ':') {
...
auto check_ip(const std::string_view ip_address, int af) -> Error
{
  ...
  if (ip_address.size() < 3) { return Error::UNKOWN_ERROR; }

  int result{};
  std::string ip;
  if (af == AF_INET6) {
    ip =
      std::string(std::next(ip_address.begin()), std::prev(ip_address.end()));
    in6_addr addr;
  ...
  -> std::optional<std::string>
{
  ...
  if (uri_str.begin() == uri_str.end()
      || std::next(uri_str.begin()) == uri_str.end() || *uri_str.begin() != '/'
      || *std::next(uri_str.begin()) != '/') {
    return {};
  }

  auto end_authority = std::find_if(
    uri_str.begin() + 2, uri_str.end(), [](const auto character) -> bool {
  ...
  if (end_authority != uri_str.end()) {
    uri_str = std::span<const char>(std::next(end_authority), uri_str.end());
  }
...
auto authority_contains_port(const std::string& authority) -> bool
{
  ...
  if (bracket != authority.end()) {
    return std::next(bracket) != authority.end() ? *std::next(bracket) == ':'
                                                 : false;
  }
  ...
  -> std::optional<std::string>
{
  ...
  if (end_user_info == authority.end()) { return {}; }
...
auto extract_port_number(const std::string& authority) -> std::uint16_t
{
  ...
  if (bracket != authority.end()) {
    ...
    port = std::string_view(
      std::next(std::ranges::find(authority, ':')), authority.end());
  }
...
auto extract_hostname(std::string_view authority) -> std::string_view
{
  ...
  const auto end_hostname =
    std::find(authority.rbegin(), authority.rend(), ':');
